...
class immigration_author_widget extends WP_Widget {
...
'immigration_author_widget', 


// Widget name will appear in UI
esc_html__( '[ Immigration ] Author Widget', 'immigration-companion' ),

// Widget description
array( 'description' => esc_html__( 'Add author content', 'immigration-companion' ), ) 
);
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'Author Name', 'immigration-companion' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'textarea' ) ); ?>"><?php esc_html_e( 'Short Descriptions:' ,'immigration-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Facebook Url' ) ); ?>"><?php esc_html_e( 'Facebook Url:' ,'immigration-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Twitter Url' ) ); ?>"><?php esc_html_e( 'Twitter Url:' ,'immigration-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Dribbble Url' ) ); ?>"><?php esc_html_e( 'Dribbble Url:' ,'immigration-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Behance Url' ) ); ?>"><?php esc_html_e( 'Behance Url:' ,'immigration-companion'); ?></label> 
...
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>"><?php _e( 'Image', 'immigration-companion' ); ?>:</label>
	<div class="immigration-media-container">
		<div class="immigration-media-inner">
			...
			<span class="immigration-no-image" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-noimg" <?php echo wp_kses_post( $no_img_style ); ?>><?php _e( 'No image selected', 'immigration-companion' ); ?></span>
		</div>
	
	<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image' ) ); ?>" value="<?php echo esc_attr( $image ); ?>" class="immigration-media-url" />

	<input type="button" value="<?php echo _e( 'Remove', 'immigration-companion' ); ?>" class="button immigration-media-remove" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-remove" <?php echo wp_kses_post( $img_style ); ?> />

	<?php $button_text = ( $image != '' ) ? __( 'Change Image', 'immigration-companion' ) : __( 'Select Image', 'immigration-companion' ); ?>
	<input type="button" value="<?php echo esc_html( $button_text ); ?>" class="button immigration-media-upload" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-button" />
	<br class="clear">
	</div>
</p>
<style>
.immigration-media-container {
	width: 98%;
}

.immigration-media-inner {
...
.immigration-media-inner img {
	max-width: 100%;
	height: auto;
}

.immigration-media-url {
	display: none;
}

.immigration-media-remove {
	float: left;
	width: 48%;
}

.immigration-media-upload {
	float: right;
	width: 48%;
}
</style>
...
jQuery(function($){
    ...
    wpshed_image_upload( '.immigration-media-upload' );
    ...
    wpshed_image_remove( '.immigration-media-remove' );
...
} // Class immigration_author_widget ends here


// Register and load the widget
function immigration_author_load_widget() {
	register_widget( 'immigration_author_widget' );
}
add_action( 'widgets_init', 'immigration_author_load_widget' );