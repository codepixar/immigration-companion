...
function immigration_import_files() {
	...
	$demoImg = '<img src="'.plugins_url( 'screen-image.png', __FILE__ ) .'" alt="'.esc_attr__( 'Demo Preview Imgae', 'immigration-companion' ).'" />';
  ...
  return array(
    array(
      ...
      'import_customizer_file_url'   => plugins_url( 'immigration-customizer.dat', __FILE__ ),
...
function immigration_import_plugin_page_setup( $default_settings ) {
	...
	$default_settings['page_title']  = esc_html__( 'One Click Demo Import' , 'immigration-companion' );
	$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'immigration-companion' );
...
function immigration_demo_import_custom_scripts(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'immigration-demo-import' ){
		// style
		wp_enqueue_style( 'immigration-demo-import', plugins_url( 'css/demo-import.css', __FILE__ ), array(), '1.0', false );
	}
